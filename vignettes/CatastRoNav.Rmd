...
-   **Vector objects:** Parcels, addresses, buildings, cadastral zones and more.
    These objects are provided by **CatastRoNav** as *sf* objects (see `?sf)`.
...
<img src="./olite-1.png" title="plot of chunk olite" alt="plot of chunk olite" width="100%" />
...
pamp_bu <- catrnav_wfs_get_buildings_bbox(pamp_buff)
```
...
dataviz <- dataviz %>%
  mutate(year = year)
#> Error in mutate(., year = year): object 'dataviz' not found
```
