test_that("BBOX Check errors", {
  expect_error(catrnav_wfs_get_address_bbox(x = "1234"))
  expect_error(catrnav_wfs_get_address_bbox(c("1234", "a", "3", "4")))
  expect_error(catrnav_wfs_get_address_bbox(c(1, 2, 3)))
  expect_error(catrnav_wfs_get_address_bbox(c(1, 2, 3, 4)))
})
...
test_that("BBOX Check projections", {
  ...
  expect_null(catrnav_wfs_get_address_bbox(c(760926, 4019259, 761155, 4019366),
    srs = 25829
  ))

  expect_message(catrnav_wfs_get_address_bbox(c(
    1071071, 4747924,
    1071171, 4748024
  ),
  srs = 25829,
  ...
  obj3 <- catrnav_wfs_get_address_bbox(bbox2)
  ...
  obj4 <- catrnav_wfs_get_address_bbox(vec, srs = 4326)
