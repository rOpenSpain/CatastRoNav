...
- User-facing messages now use **cli** for consistent, informative
  output.
- This refactor was completed with AI assistance and reviewed by the
  package maintainer.
- Network requests now use **httr2** with retries. Use
  `options(catastronav_timeout = 300)` to set the timeout and
  `options(catastronav_ssl_verify = 1L)` to control SSL verification.
  These settings fall back to `catastro_timeout` and
  `catastro_ssl_verify`, respectively. Requests now detect offline
  sessions before downloading and failed requests return `NULL` instead
  of stopping execution.
- Spatial files now use the same UTF-8 normalization strategy as
  **CatastRo**, removing the **stringi** dependency and avoiding lossy
  transliteration.
...
- Added ATOM capabilities for buildings and addresses
...
- Added WMS INSPIRE support with
  ...
  using **mapSpain** and **terra**. It retrieves address, building and
  cadastral parcel layers, supports the `ELFCadastre` parcel style and
  accepts additional **mapSpain** request settings through `options`.
  Offline or failed requests return `NULL`.
...
- [`catrnav_atom_get_address()`](https://ropenspain.github.io/CatastRoNav/reference/catrnav_atom_get_address.md),
  ...
  now select the closest municipality when a pattern matches multiple
  names and report the alternatives. Calls with `cache = FALSE` use
  temporary files and invalid arguments produce informative errors.
- [`catrnav_atom_search_munic()`](https://ropenspain.github.io/CatastRoNav/reference/catrnav_atom_search_munic.md)
  now searches the ATOM index by municipality name or cadastral code and
  returns all matches ordered by proximity.
- [`catrnav_clear_cache()`](https://ropenspain.github.io/CatastRoNav/reference/catrnav_clear_cache.md)
  now validates its arguments and reports the size of deleted cached
  data when `verbose = TRUE`.
- [`catrnav_detect_cache_dir()`](https://ropenspain.github.io/CatastRoNav/reference/catrnav_set_cache_dir.md)
  now reports the active cache directory.
- [`catrnav_set_cache_dir()`](https://ropenspain.github.io/CatastRoNav/reference/catrnav_set_cache_dir.md)
  now defaults `cache_dir` to `NULL`, validates its arguments and stores
  persistent configuration under
  [`tools::R_user_dir()`](https://rdrr.io/r/tools/userdir.html).
  Existing configuration is migrated automatically from the previous
  location and is reliably restored in later sessions. Set
  `cache_dir = FALSE` to use a nonpersistent temporary cache even when
  `install = TRUE`.
...
- Added a `count` argument that sets the maximum number of features to
  be returned.
- **New features:**
